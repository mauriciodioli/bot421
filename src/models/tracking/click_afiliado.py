...
click_afiliado = Blueprint('click_afiliado', __name__)

class ClickAfiliado(db.Model):
    __tablename__ = "click_afiliado"

    id = db.Column(db.Integer, primary_key=True)

    # Claves / referencias (agrega FK si existen esas tablas)
    ...
    destino    = db.Column(db.Text)                                      # URL final con subIDs
    ...
    def __repr__(self):
        return (f"<ClickAfiliado id={self.id} pub={self.publicacion_id} "
                f"user={self.user_id} visitor={self.visitor_id} ts={self.creado_en}>")
    @classmethod
    def crear_tabla_click_afiliado(self):
        # Verificar si la tabla 'click_afiliado' existe antes de crearla
        insp = inspect(db.engine)
        if not insp.has_table("click_afiliado"):
            db.create_all()


class ClickAfiliadoSchema(SQLAlchemyAutoSchema):
    class Meta:
        model = ClickAfiliado
        load_instance = True
        include_fk = True
