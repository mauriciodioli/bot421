...
def obtener_unidad_trader_por_trigger(idTrigger):
    return db.session.query(UnidadTrader).filter(UnidadTrader.trigger_id == idTrigger).first()

def filtrar_coincidencias(ContenidoSheet_list, diccionario_operaciones_enviadas, símbolos_vistos, account, app):
    coincidencias = []
    for i, elemento1 in enumerate(ContenidoSheet_list):
        if i >= 2:
            elemento1 = list(elemento1) if not isinstance(elemento1, list) else elemento1
            for key, elemento2 in diccionario_operaciones_enviadas.items():
                if elemento1[0] == elemento2['Symbol'] and elemento2['Symbol'] not in símbolos_vistos:
                    if elemento1[4] == 'closed.' or (elemento1[2] == 'SHORT' and elemento1[4] == '') or (elemento1[4] == 'OPEN.' and simbolo_no_en_diccionario(elemento1[0], diccionario_operaciones_enviadas)):
                        app.logger.info(f'account: {account}, elemento1[0]: {elemento1[0]}, elemento2[_ut_]: {elemento2["_ut_"]}, elemento1[4]: {elemento1[4]}, tipo: {elemento1[1]}, tradeEnCurso: {elemento1[2]}')
                        elemento1[3] = int(elemento2['_ut_'])
                        coincidencias.append(elemento1)
                    símbolos_vistos.add(elemento2['Symbol'])
    return coincidencias

def procesar_coincidencias(ContenidoSheet_list, message, coincidencias, unidadTrader):
    for i, elemento1 in enumerate(ContenidoSheet_list):
        if i >= 2:
            for elemento2 in message:
                if elemento1[2] == 'LONG_' and int(elemento1[3]) != 0 and elemento1[4] == 'OPEN.' and simbolo_no_en_diccionario(elemento1[0], diccionario_operaciones_enviadas) and elemento1[0] == elemento2:
                    ...
                    coincidencias.append(tuple(lista_modificable))
    return coincidencias

def convertir_cadena_a_float(cadena):
    cadena_sin_puntos = cadena.replace('.', '')
    cadena_correcta = cadena_sin_puntos.replace(',', '.')
    return float(cadena_correcta)
...
def procesar_elemento(elemento, unidadTrader, usuariodb, account, tiempoLecturaSaldo, app):
    ...
    if ut > 0:
        ...
        senial = 'closed.' if elemento[2] == 'SHORT' else elemento[4]
        ...
        if elemento[0] in diccionario_global_operaciones:
            contenido = diccionario_global_operaciones[elemento[0]]
            campos_especificos = ['symbol', 'tipo_de_activo', 'tradeEnCurso', 'ut', 'senial', 'status']
            contenido_linea = ', '.join([f"{campo}: {contenido[campo]}" for campo in campos_especificos])
            app.logger.info(f'c: {account}, {contenido_linea}')
        else:
            app.logger.info(f"No se encontró contenido para {elemento[0]} en diccionario_global_operaciones.")

def manejar_sin_operaciones_pendientes(account, idTrigger):
    parametros = {
        'account': get.ConexionesBroker[account]['cuenta'],
        'user_id': idUser,
        'symbol': '',
